...
import dif_test_dataset_2_copy as dif_util
...
class DIF:
    ...
    #--------------------------------modified-------------------------------------------------------------
    def decision_function_custom(self, X):
        """Predict raw anomaly score of X using the fitted detector.
        The anomaly score of an input sample is computed based on different
        detector algorithms. For consistency, outliers are assigned with
        larger anomaly scores.
        ...
        anomaly_scores : numpy array of shape (n_samples,)
            The anomaly score of the input samples.
        """
        # Z is the dataset that gets evaluated and conditionally added as new nodes to the existing trees
        # print("---------------------------Inside modified decision_function-------------------------")
        # print("type(X): ", type(X))
        # print("X.shape: ", X.shape)
        test_reduced_lst = self._inference_transfer(X)
        #new_test_reduced_list = test_reduced_lst
        # print("type(test_reduced_lst): ", type(test_reduced_lst))
        # print("len(test_reduced_lst): ", len(test_reduced_lst))
        # print("test_reduced_lst[0].shape: ", test_reduced_lst[0].shape)
        #online_train_reduced_lst = self._inference_transfer(Z)
        random_row_indices = np.random.choice(test_reduced_lst[0].shape[0], size=500)
        online_train_reduced_ndarray = test_reduced_lst[0][random_row_indices]
        # random_row_indices = np.random.choice(test_reduced_lst[0].shape[0], size=500,replace=False)
        # new_test_reduced_list[0] = test_reduced_lst[0][random_row_indices, :]
        # print("online_random_train_data.shape: ",online_train_reduced_ndarray.shape)
        # print("online_random_train_data: ",online_train_reduced_ndarray)
        final_scores = self._inference_scoring_custom(test_reduced_lst, online_train_reduced_ndarray, n_processes=self.n_processes)
        #final_scores = self._inference_scoring(new_test_reduced_list, online_train_reduced_ndarray, n_processes=self.n_processes)
        return final_scores
    ...
    #-------------------------------------------MODIFIED-----------------------------------------------
    def _inference_scoring_custom(self, x_reduced_lst, z_reduced_lst, n_processes):
        print("--------------Inside inference_scoring modified-------------------------------------")
        if self.new_score_func:
            #score_func = self.single_predict
            score_func = self.single_predict_custom
        else:
            score_func = self.single_predict_abla
        ...
        self.score_lst = np.zeros([self.n_ensemble, n_samples])
        if n_processes == 1:
            # print("self.n_ensemble: ", self.n_ensemble)
            # print("len(x_reduced_lst): ", len(x_reduced_lst))
            # print("x_reduced_lst[0].shape: ", x_reduced_lst[0].shape)
            for i in range(self.n_ensemble):
                # print("x_reduced_lst[i]: ", x_reduced_lst[i])
                # print("self.clf_lst[i]: ", self.clf_lst[i])
                # print("i: ",i)
                # print("x_reduced_lst[i].shape: ", x_reduced_lst[i].shape)
                scores = score_func(x_reduced_lst[i], z_reduced_lst, self.clf_lst[i])
                self.score_lst[i] = scores
        else:
            # multiprocessing predict
            start = np.arange(0, self.n_ensemble, np.ceil(self.n_ensemble / n_processes))
        ...
        # print("len(self.score_lst)", len(self.score_lst))
        final_scores = np.average(self.score_lst, axis=0)
    ...
    #-------------------------------------MODIFIED---------------------------------------------------
    @staticmethod
    def single_predict_custom(x_reduced, z_reduced, clf):
        #scores = _cal_score(x_reduced, clf)
        scores = _cal_score_custom(x_reduced, z_reduced, clf)
        return scores
...
#--------------any experiments with the original (such as printing some values etc.) sould be done here-------
def _cal_score(xx, clf):
    depths = np.zeros((xx.shape[0], len(clf.estimators_)))
    depth_sum = np.zeros(xx.shape[0])
    deviations = np.zeros((xx.shape[0], len(clf.estimators_)))
    ...
    for ii, estimator_tree in enumerate(clf.estimators_):
        ...
        #-------------run online dif with new training data zz-----------------------------------    
        # dif_util.fit_online(tree, zz)
        # print("n_node: ", n_node)

        # get feature and threshold of each node in the iTree
        # in feature_lst, -2 indicates the leaf node
        feature_lst, threshold_lst = tree.feature.copy(), tree.threshold.copy()
        # print("feature_lst.shape: ", feature_lst.shape)
        # print("threshold_lst.shape: ", threshold_lst.shape)
        ...
        leaves_index = estimator_tree.apply(xx)
        node_indicator = estimator_tree.decision_path(xx)
        # print("node_indicator.shape: ", node_indicator.shape)
...
#newly written custom dif 
def _cal_score_custom(xx, zz, clf):
        # print("------------------In custom _cal_score method--------------------------")
        depths = np.zeros((xx.shape[0], len(clf.estimators_)))
        depth_sum = np.zeros(xx.shape[0])
        deviations = np.zeros((xx.shape[0], len(clf.estimators_)))
        leaf_samples = np.zeros((xx.shape[0], len(clf.estimators_)))

        offline_max_samples = clf.max_samples
        # print("max_samples (before): ", clf.max_samples)
        for ii, estimator_tree in enumerate(clf.estimators_):
            # estimator_population_ind = sample_without_replacement(n_population=xx.shape[0], n_samples=256,
            #                                                       random_state=estimator_tree.random_state)
            # estimator_population = xx[estimator_population_ind]
            max_samples = offline_max_samples
            tree = estimator_tree.tree_
            # n_node = tree.node_count
            # print("n_node(tree.node_count): ", n_node)

            #-------------run online dif with new training data zz-----------------------------------    
            root_node, count, n_new_inserts, new_data_lst = dif_util.fit_online(tree, zz)
            # print("fit online method run and returned to calc_score method...")
            n_node = count
            # print("n_node: ", n_node)
            #dif_util.print_tree_with_feature(root_node)
            max_samples = max_samples + n_new_inserts
            #-----------------part-of-custom-implementation-------------------------------------------------
            # root_node = dif_util.TreeNode()
            # root_node.node_id = 0
            # #root_node.feature = clf.estimators_[0].tree_.feature[0]
            # root_node.feature = tree.feature.copy()
            # #root_node.threshold = clf.estimators_[0].tree_.threshold[0]
            # root_node.threshold = tree.threshold.copy()
            # count = dif_util.build_tree_2(root_node, tree.children_left, tree.children_right, tree.feature, tree.threshold)
            # n_node = count
            # print("n_node(custom): ", n_node)
            #------------------------------------------------------------------------------------------------
            ...
            # in feature_lst, -2 indicates the leaf node
            # feature_lst, threshold_lst = tree.feature.copy(), tree.threshold.copy()
            # print("feature_lst (original): ", feature_lst)
            # # print("type(feature_lst) - original: ", type(feature_lst))
            # print("threshold_lst (original): ", threshold_lst)
            #print("type(threshold_lst) - original: ", type(threshold_lst))
            #----------------------part-of-custom-implementation--------------------------------------------
            # print("About to call dif_util.get_feature_and_threshold(root_node)...")
            feature_lst, threshold_lst = dif_util.get_feature_and_threshold(root_node)   
            # print("Calling dif_util.get_feature_and_threshold(root_node) ends") 
            # print("feature_lst.shape: ",feature_lst.shape)
            # print("threshold_lst.shape: ",threshold_lst.shape)
            #print("feature_lst (custom): ", feature_lst)
            # print("type(feature_lst) - custom: ", type(feature_lst))
            #print("threshold_lst (custom): ", threshold_lst)
            # print("type(threshold_lst) - custom: ", type(threshold_lst))
            #------------------------------------------------------------------------------------------------
            ...
            # compute depth and score
            # leaves_index = estimator_tree.apply(xx)
            # print("leaves_index (original): ", leaves_index)
            # print("type(leaves_index) - original: ", type(leaves_index))
            # node_indicator = estimator_tree.decision_path(xx)
            # print("node_indicator (original): ", node_indicator)
            # print("type(node_indicator) - original: ", type(node_indicator))

            #----------------------part-of-custom-implementation--------------------------------------------
            leaves_index = dif_util.apply(root_node, xx)
            # print("leaves_index (custom): ", leaves_index)
            # print("type(leaves_index) - custom: ", type(leaves_index))
            node_indicator = dif_util.decision_path(root_node, n_node, xx)
            # print("node_indicator (custom): ", node_indicator)
            # print("type(node_indicator) - custom: ", type(node_indicator))
            # print("node_indicator.shape - custom: ", node_indicator.shape)
            #-----------------------------------------------------------------------------------------------

            # The number of training samples in each test sample leaf
            # n_node_samples = estimator_tree.tree_.n_node_samples
            # print("n_node_samples (original): ", n_node_samples)
            # print("type(n_node_samples) - original: ", type(n_node_samples))
            # print("n_node_samples.shape - original: ", n_node_samples.shape)

            # node_indicator is a sparse matrix with shape (n_samples, n_nodes), indicating the path of input data samples
            # each layer would result in a non-zero element in this matrix,
            # and then the row-wise summation is the depth of data sample
            #n_samples_leaf = estimator_tree.tree_.n_node_samples[leaves_index]
            # print("n_samples_leaf (original): ", n_samples_leaf)
            # print("type(n_samples_leaf) - original: ", type(n_samples_leaf))
            #----------------------part-of-custom-implementation--------------------------------------------
            n_node_samples = dif_util.get_n_node_samples(root_node, n_node, xx)
            n_node_samples = dif_util.get_n_node_samples_2(root=root_node, new_data_lst=new_data_lst,n_node_samples=n_node_samples)
            # print("n_node_samples (custom): ", n_node_samples)
            # print("type(n_node_samples) - custom: ", type(n_node_samples))
            # print("n_node_samples.shape - custom: ", n_node_samples.shape)

            n_samples_leaf = n_node_samples[leaves_index]
            # print("n_samples_leaf (custom): ", n_samples_leaf)
            # print("type(n_samples_leaf) - custom: ", type(n_samples_leaf))
            #-----------------------------------------------------------------------------------------------
            d = (np.ravel(node_indicator.sum(axis=1)) + _average_path_length(n_samples_leaf) - 1.0)
            depths[:, ii] = d
            depth_sum += d
            ...
            # print("value_mat.shape: ", value_mat.shape)    
            # print("th_mat.shape: ", th_mat.shape)    
        ...
        # print("max_samples (after): ", max_samples)
        # print("len(clf.estimators_): ", len(clf.estimators_))
        scores = 2 ** (-depth_sum / (len(clf.estimators_) * _average_path_length([max_samples])))
        #----------------------part-of-custom-implementation-------------------------------------------------
        scores = 2 ** (-depth_sum / (len(clf.estimators_) * _average_path_length([max_samples])))
        #----------------------------------------------------------------------------------------------------
        deviation = np.mean(deviations, axis=1)
        leaf_sample = (max_samples - np.mean(leaf_samples, axis=1)) / max_samples
