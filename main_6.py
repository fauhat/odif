...
#---------------------For Single Tree Experiment(MUST BE COMMENTED OUT for multi-tree experiment)----------------------------------
model_configs['n_ensemble'] = 1
model_configs['n_estimators'] = 1
#----------------------------------------------------------------------------------------------------------
...
for f in data_lst:
    ...
    auc_lst_online, ap_lst_online = np.zeros(args.runs), np.zeros(args.runs),
    ...
    for i in range(args.runs):
        ...
        auc_online, ap_online = utils.evaluate(y, scores)
        auc_lst_online[i], ap_lst_online[i] = auc_online, ap_online
        t1_lst[i] = t1 - start_time

        print(f'{dataset_name}, {auc_lst_online[i]:.4f}, {ap_lst_online[i]:.4f}, {t1_lst[i]:.1f}, {args.model}')
    ...
    avg_auc_online, avg_ap_online = np.average(auc_lst_online), np.average(ap_lst_online)
    std_auc, std_ap = np.std(auc_lst), np.std(ap_lst)
    std_auc_online, std_ap_online = np.std(auc_lst_online), np.std(ap_lst_online)
    ...
    f = open(result_file, 'a')
    txt = f'{dataset_name}, {avg_auc_online:.4f}, {std_auc_online:.4f}, ' \
          f'{avg_ap_online:.4f}, {std_ap_online:.4f}, ' \
          f'{avg_time:.1f}, cont, {args.contamination}'
    print(txt, file=f)
    print("-----online------")
    print(txt)
